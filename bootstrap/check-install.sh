...
check_resource_flavor() {
    ...
    do
        ...
        ((i=i+1))
        if [ $i -gt 200 ]; then
            echo -e "🕱${RED}Failed - nvidia-gpu-flavor never ready - $RF?.${NC}"
            exit 1
        fi
        sleep 10
...
check_llm_pods() {
    ...
    do
        ...
        if [ "$LLAMA_STATUS" == "True" ] && [ "$i" -gt 50 ]; then
            ...
            oc -n llama-serving annotate --overwrite inferenceservice deepseek-r1-0528-qwen3-8b-bnb-4bit serving.kserve.io/stop="false"
        fi
        PODS=$(oc get pods -n llama-serving | grep -e Running | grep -e 2/2 | wc -l)
    done
